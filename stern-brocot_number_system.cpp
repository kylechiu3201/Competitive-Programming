...
int main() {
    ...
    while(1) {
        ...
        double find = (double)num/den, cur;
        pii node = {1, 1}, parl = {0, 1}, parr = {1, 0};
        string ans;
        while(!(node.fi == num && node.se == den)) {
            cur = (double)node.fi/node.se;
            if(cur > find) {
                ans += "L";
                parr.fi = node.fi, parr.se = node.se;
                node.fi += parl.fi, node.se += parl.se;
            }
            else {
                ans += "R";
                parl.fi = node.fi, parl.se = node.se;
                node.fi += parr.fi, node.se += parr.se;
            }
        }
        cout << ans << NEWL;
    }
